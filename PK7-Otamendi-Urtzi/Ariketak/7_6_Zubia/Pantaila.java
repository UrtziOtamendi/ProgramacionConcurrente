...
class Pantaila {
	...
	synchronized public boolean mugituGorria(int i) throws InterruptedException{
		int X = gorriaX[i];
		if      (X==zab && gorriaX[(i+1)%maxg]!=0)   X=0;			//Bukaerara iristean
		else if (X!=zab && gorriaX[(i+1)%maxg]!=X+1) X=X+1;		//Beste posizioetan
		if (gorriaX[i]!=X) { gorriaX[i]=X; pantailaratu();}			//Mugitu ahal bada, mugitu eta pantailaratu
	...
	synchronized public boolean mugituUrdina(int i) throws InterruptedException{
		int X = urdinaX[i];
		if      (X==0 && urdinaX[(i+1)%maxu]!=zab)  X=zab;
		else if (X!=0 && urdinaX[(i+1)%maxu]!=X-1 ) X=X-1;
		if (urdinaX[i]!=X) { urdinaX[i]=X; pantailaratu();}
